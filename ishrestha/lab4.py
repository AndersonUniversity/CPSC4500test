...
def numCheck(num):
   ...
   if a !=3: #making sure there are only 3 digits
    return False
       ...
       if int(b[0])>int(b[1]) and int(b[1])>int(b[2]): #making sure they are in decreasing order
...
def numConvert(a):
    ...
    if numCheck(b): #checks if it is of 3 length and in decreasing order
        ...
        newNum1= a - newNum # Orginal number - reversed number of original number
    ...
    else: #if less or greater than 3 digits and digits are not in decreasing order
